...
use namada_core::ledger::ibc::actions::prefixed_coin;
use thiserror::Error;
    ...
    CA: 'static + WasmCacheAccess,
{
    ...
    fn validate_tx(
        ...
        match &ibc_msg {
            IbcMessage::Ics20(msg) => self.validate_sending_token(msg),
    ...
    CA: 'static + WasmCacheAccess,
{
    fn validate_sending_token(&self, msg: &MsgTransfer) -> Result<bool> {
        let denom = match ibc_storage::token_hash_from_denom(&msg.token.denom)
            .map_err(|e| {
        ...
        let token = ibc_storage::token(&coin)
            .map_err(|e| Error::Denom(e.to_string()))?;
        let amount = ibc_storage::amount(&coin)
            .map_err(|e| Error::Denom(e.to_string()))?;
    ...
    fn validate_receiving_token(&self, packet: &Packet) -> Result<bool> {
        ...
        let token = ibc_storage::token(&data.token)
            .map_err(|e| Error::Denom(e.to_string()))?;
        let amount = ibc_storage::amount(&data.token)
            .map_err(|e| Error::Denom(e.to_string()))?;
