    ...
    H: StorageHasher + Sync + 'static,
{
    ...
    pub fn init_chain(
        ...
        } in genesis.token_accounts
        {
            ...
            parameters.init_storage(&address, &mut self.storage);
            self.storage
                .write(
                    &token::last_inflation(&address),
                    types::encode(&last_inflation),
                )
                .unwrap();
            self.storage
                .write(
                    &token::last_locked_ratio(&address),
                    types::encode(&last_locked_ratio),
                )
                .unwrap();
            ...
            let mut total_balance_for_token = Amount::default();
            ...
            self.storage
                .write(
                    ...
                    total_balance_for_token
                        .try_to_vec()
                        .expect("encoding amount should not fail"),
                )
                .unwrap();
        }
